...
var iQontrolRoles = {
	...
	"iQontrolWidget": 				{
										...
										options: {
											SECTION_ICONS: {name: "Icons", type: "section"},
											icon_on: {name: "Icon on", type: "icon", defaultIcons: "widget_on.png", default: ""},
											icon_off: {name: "Icon off", type: "icon", defaultIcons: "widget_off.png", default: ""},
											SECTION_GENERAL: {name: "General", type: "section"},
											readonly: {name: "Readonly", type: "checkbox", default: "false"}, 
											invertUnreach: {name: "Invert UNREACH (use connected instead of unreach)", type: "checkbox", default: "false"}, 
											SECTION_BATTERY: {name: "BATTERY Empty Icon", type: "section"},
		...
		if(stateIndex > -1){ //State exists in config (value ist stored in .value)
			...
			if (stateObject && typeof stateObject.value != udef){
				if(stateObject.commonRole == 'const' || stateObject.commonRole == 'array') { //role of state is 'const' or 'array'
					...
					//--Special: If STATE of iQontrolWidget is empty, it is filled with Virtual
					if(device.commonRole == "iQontrolWidget" && linkedStateId == ""){
...
function renderView(viewId, triggeredByReconnection){
	...
	fetchView(actualViewId, function(){
		...
		if (actualView.nativeBackgroundImage) {
			...
			if(!triggeredByReconnection) window.scrollTo(0, 0);
		}
		//Render View
		...
		for (var deviceIndex = 0; deviceIndex < actualView.devices.length; deviceIndex++){
			...
			viewContent += "<div data-groups='" + device.commonRole + "' class='viewShuffleTile iQontrolDevicePressureIndicator" + ((getDeviceOptionValue(device, "hideDeviceIfInactive") == "true")?" hideDeviceIfInactive":"") + "' " + ((getDeviceOptionValue(device, "hideDeviceIfInactive") == "true")?"style='visibility: hidden;' ":"") + "data-iQontrol-Device-ID='" + deviceIdEscaped + "'>";
				//--Box 
				viewContent += "<div class='iQontrolDevice" + (getDeviceOptionValue(device, "sizeInactive") ? " " + getDeviceOptionValue(device, "sizeInactive") : "") + (getDeviceOptionValue(device, "sizeActive") ? " " + getDeviceOptionValue(device, "sizeActive") : "") + "' data-iQontrol-Device-ID='" + deviceIdEscaped + "'>";
					...
					if (deviceLinkedStateIds["URL"]){
						...
						switch(device.commonRole){
							...
							if(deviceLinkedStateIds["STATE"]) onclick = "toggleState(\"" + deviceLinkedStateIds["STATE"] + "\", \"" + deviceIdEscaped + "\");";
							linkContent += "<a class='iQontrolDeviceLinkToToggle' data-iQontrol-Device-ID='" + deviceIdEscaped + "' onclick='" + onclick + "'>";
								if (icons["on"] !== "none") iconContent += "<image class='iQontrolDeviceIcon on' data-iQontrol-Device-ID='" + deviceIdEscaped + "' src='" + (icons["on"] || "./images/icons/widget_on.png") + "' " + (variableSrc["on"] ? "data-variablesrc='" + variableSrc["on"] + "' " : "") + "/>";
								if (icons["off"] !== "none") iconContent += "<image class='iQontrolDeviceIcon off active' data-iQontrol-Device-ID='" + deviceIdEscaped + "' src='" + (icons["off"] || "./images/icons/widget_off.png") + "' " + (variableSrc["off"] ? "data-variablesrc='" + variableSrc["off"] + "' " : "") + "/>";
							break;
...
function resizeFullWidthDevicesToFitScreen(){
	...
	if(options.LayoutViewResizeDevicesToFitScreenOnBigScreens || screenWidth <= (options.LayoutViewResizeDevicesToFitScreenTreshold || 600)){
		...
		customCSS = ".iQontrolDevice.fullWidth, .iQontrolDevice:not(.active).fullWidthIfInactive, .iQontrolDevice.active.fullWidthIfActive {";
		...
		customCSS += "}";
		customCSS += ".iQontrolDevice.fullHeight, .iQontrolDevice:not(.active).fullHeightIfInactive, .iQontrolDevice.active.fullHeightIfActive {";
