...
function renderSettings(){
	...
	$("#SettingsAddDeviceObjectBrowser").html("");
	var settingsAddDeviceObjectBrowserSelectedId = "";
	var SettingsAddDeviceExistingDevicesBrowserSelectedId = "";
	var usedObjectsSorted = [];
	for (id in usedObjects){
		usedObjectsSorted.push(id);
	}
	usedObjectsSorted.sort();
	for(i = 0; i < usedObjectsSorted.length; i++){
		var id = usedObjectsSorted[i];
		var commonName = "";
		if (usedObjects[id] && typeof usedObjects[id].common.name !== udef && usedObjects[id].common.name !== "") commonName = "<p>||" + usedObjects[id].common.name + "</p>";
		$("#SettingsAddDeviceObjectBrowser").append("<li data-icon='check'><a href='' onclick='settingsAddDeviceObjectBrowserSelectedId=\"" + id + "\";'>" + id + commonName + "</a></li>");
	}
	$("#SettingsAddDeviceObjectBrowser").listview("option", "autodividersSelector", function(elt){ //Custom Autodividers
		var text = $.trim( elt.text() ) || null;
		if(!text) return null;
		text = text.substring(0, text.lastIndexOf("||")); //remove common.name if given
		text = text.substring(0, text.lastIndexOf(".")); //remove common.name if given
		return text;
	});
	$("#SettingsAddDeviceObjectBrowser").listview("refresh");
	//--Collapsible List Dividers
	$(document).on("click", '#SettingsAddDeviceObjectBrowser .ui-li-divider', function (e) {
	...
	//Last actions of rendering
	$(".SettingsAddDeviceState").hide();
	$(".settingsToBeRendered").show();
	$("#SettingsAddDeviceCommonRole").on('change', settingsAddDeviceCommonRoleChanged);
}

function settingsAddView(){
	if ($("#SettingsAddViewCommonName").val() == ""){
		alert("ERROR: You need at least to give the View a name...");
		return;
	}
	var objId = namespace + ".Views." + $("#SettingsAddViewCommonName").val()
	if(usedObjects[objId]) if(!confirm("View exits - overwrite?")) return;
	var obj = {
		"_id": objId,
		"type": "channel",
		"common": {
			...
			"role": "iQontrolView",
			"name": $("#SettingsAddViewCommonName").val(),
			"icon": ""
		},
		"native": {
	...
	setObject(objId, obj, function(error){
		if (error) alert("Error at creating object"); else {
			getEverything();
			alert ("Object created. Database is reloading now. That may take a few seconds.");
		}
	});
}

function settingsAddDeviceExistingDevicesBrowserViewChanged(){
	var selectedView = $("#SettingsAddDeviceExistingDevicesBrowserViewSelector").val();
	$("#SettingsAddDeviceExistingDevicesBrowser").html("");
	for (id in usedObjects){ //Get existing Devices for each View
		if(id.indexOf(selectedView) == 0 && id.lastIndexOf(".") == selectedView.length){ //begins with views id and the last dot is exactly after views id
			$("#SettingsAddDeviceExistingDevicesBrowser").append("<li data-icon='check'><a href='' onclick='SettingsAddDeviceExistingDevicesBrowserSelectedId=\"" + id + "\";'>" + id + "</a></li>");
		}
	}
	$("#SettingsAddDeviceExistingDevicesBrowser").listview("refresh");
}

function settingsAddDeviceExistingDevicesBrowserButtonCopySelected(){
	var selectedId = SettingsAddDeviceExistingDevicesBrowserSelectedId;
	if(selectedId == ""){
		alert("Nothing selected");
		return;
	}
	$("#SettingsAddDeviceCommonName").removeClass("matched");
	if(typeof usedObjects[selectedId] !== udef && typeof usedObjects[selectedId].common.name !== udef){
		$("#SettingsAddDeviceCommonName").val(usedObjects[selectedId].common.name);
		$("#SettingsAddDeviceCommonName").addClass("matched");
	}
	var childStates = [];
	for(id in states){
		if(id.indexOf(selectedId) == 0) childStates.push(id);
	}
	childStates.sort().reverse();
	$("#SettingsAddDeviceStateSTATE").removeClass("matched");
	$("#SettingsAddDeviceStateLEVEL").removeClass("matched");
	$("#SettingsAddDeviceStateHUE").removeClass("matched");
	$("#SettingsAddDeviceStateSET_TEMPERATURE").removeClass("matched");
	$("#SettingsAddDeviceStateHUMIDITY").removeClass("matched");
	$("#SettingsAddDeviceStateTEMPERATURE").removeClass("matched");
	$("#SettingsAddDeviceStateCONTROL_MODE").removeClass("matched");
	$("#SettingsAddDeviceStatePARTY_TEMPERATURE").removeClass("matched");
	$("#SettingsAddDeviceStateVALVE_STATES").removeClass("matched");
	$("#SettingsAddDeviceStateLOCK_STATE").removeClass("matched");
	$("#SettingsAddDeviceStateLOCK_STATE_UNCERTAIN").removeClass("matched");
	$("#SettingsAddDeviceStateLOCK_OPEN").removeClass("matched");
	$("#SettingsAddDeviceStateBATTERY").removeClass("matched");
	$("#SettingsAddDeviceStateUNREACH").removeClass("matched");
	$("#SettingsAddDeviceStateERROR").removeClass("matched");
	for(i = 0; i < childStates.length; i++){
		var id = childStates[i];
		var stateName = id.substring(id.lastIndexOf(".") + 1, id.length);
		var linkedId = states[id].val;
		$("#SettingsAddDeviceState" + stateName).val(linkedId);
		$("#SettingsAddDeviceState" + stateName).addClass("matched");
	}
	$("#SettingsAddDeviceCommonRole").val(usedObjects[SettingsAddDeviceExistingDevicesBrowserSelectedId].common.role).selectmenu("refresh");
	settingsAddDeviceCommonRoleChanged();
}

function settingsAddDeviceObjectBrowserButtonMatchSelected(){
	var selectedId = settingsAddDeviceObjectBrowserSelectedId;
	if(selectedId == ""){
		alert("Nothing selected");
		return;
	}
	...
	for(i = 0; i < childStates.length; i++){
		...
		switch(stateName){
			case ".STATE":
			if(typeof usedObjects[id] !== udef && typeof usedObjects[id].common.role != udef && usedObjects[id].common.role == "switch.lock"){
				$("#SettingsAddDeviceStateLOCK_STATE").val(id);
				$("#SettingsAddDeviceStateLOCK_STATE").addClass("matched");
			} else {
				$("#SettingsAddDeviceStateSTATE").val(id);
				$("#SettingsAddDeviceStateSTATE").addClass("matched");
			}
			break;

			case ".LEVEL":
			$("#SettingsAddDeviceStateLEVEL").val(id);
			$("#SettingsAddDeviceStateLEVEL").addClass("matched");
			break;

			case ".HUE":
			$("#SettingsAddDeviceStateHUE").val(id);
			$("#SettingsAddDeviceStateHUE").addClass("matched");
			break;

			case ".SET_TEMPERATURE":
			$("#SettingsAddDeviceStateSET_TEMPERATURE").val(id);
			$("#SettingsAddDeviceStateSET_TEMPERATURE").addClass("matched");
			break;

			case ".HUMIDITY": case ".ACTUAL_HUMIDITY":
			$("#SettingsAddDeviceStateHUMIDITY").val(id);
			$("#SettingsAddDeviceStateHUMIDITY").addClass("matched");
			break;

			case ".TEMPERATURE": case ".ACTUAL_TEMPERATURE":
			$("#SettingsAddDeviceStateTEMPERATURE").val(id);
			$("#SettingsAddDeviceStateTEMPERATURE").addClass("matched");
			break;

			case ".CONTROL_MODE":
			$("#SettingsAddDeviceStateCONTROL_MODE").val(id);
			$("#SettingsAddDeviceStateCONTROL_MODE").addClass("matched");
			break;

			case ".PARTY_TEMPERATURE":
			$("#SettingsAddDeviceStatePARTY_TEMPERATURE").val(id);
			$("#SettingsAddDeviceStatePARTY_TEMPERATURE").addClass("matched");
			break;

			case ".STATE_UNCERTAIN":
			$("#SettingsAddDeviceStateLOCK_STATE_UNCERTAIN").val(id);
			$("#SettingsAddDeviceStateLOCK_STATE_UNCERTAIN").addClass("matched");
			break;

			case ".OPEN":
			$("#SettingsAddDeviceStateLOCK_OPEN").val(id);
			$("#SettingsAddDeviceStateLOCK_OPEN").addClass("matched");
			break;

			case ".LOWBAT":
			$("#SettingsAddDeviceStateBATTERY").val(id);
			$("#SettingsAddDeviceStateBATTERY").addClass("matched");
			break;

			case ".UNREACH":
			$("#SettingsAddDeviceStateUNREACH").val(id);
			$("#SettingsAddDeviceStateUNREACH").addClass("matched");
			break;

			case ".ERROR": case ".FAULT_REPORTING":
			$("#SettingsAddDeviceStateERROR").val(id);
			$("#SettingsAddDeviceStateERROR").addClass("matched");
			break;
		}
	}
	...
	//--all the others
	var role = null;
	if($("#SettingsAddDeviceStateLEVEL").val() !== ""){
		childId = $("#SettingsAddDeviceStateLEVEL").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	} else if($("#SettingsAddDeviceStateLOCK_STATE").val() !== ""){
		childId = $("#SettingsAddDeviceStateLOCK_STATE").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	} else if($("#SettingsAddDeviceStateSTATE").val() !== ""){
		childId = $("#SettingsAddDeviceStateSTATE").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	} else  if($("#SettingsAddDeviceStateSET_TEMPERATURE").val() !== ""){
		childId = $("#SettingsAddDeviceStateSET_TEMPERATURE").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	} else  if($("#SettingsAddDeviceStateHUMIDITY").val() !== ""){
		childId = $("#SettingsAddDeviceStateHUMIDITY").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	} else  if($("#SettingsAddDeviceStateTEMPERATURE").val() !== ""){
		childId = $("#SettingsAddDeviceStateTEMPERATURE").val();
		if(typeof usedObjects[childId] !== udef && typeof usedObjects[childId].common.role !== udef){
			role = usedObjects[childId].common.role;
		}
	}
	switch(role){
		...
		$("#SettingsAddDeviceCommonRole").val("iQontrolSwitch").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "switch.light": case "level.dimmer":
		$("#SettingsAddDeviceCommonRole").val("iQontrolLight").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "---missing---":
		$("#SettingsAddDeviceCommonRole").val("iQontrolFan").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "---missing---":
		$("#SettingsAddDeviceCommonRole").val("iQontrolThermostat").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "level.temperature":
		$("#SettingsAddDeviceCommonRole").val("iQontrolHomematicThermostat").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "---missing---":
		$("#SettingsAddDeviceCommonRole").val("iQontrolDoor").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "switch.lock":
		$("#SettingsAddDeviceCommonRole").val("iQontrolDoorWithLock").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "state":
		$("#SettingsAddDeviceCommonRole").val("iQontrolWindow").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "---missing---":
		$("#SettingsAddDeviceCommonRole").val("iQontrolValue").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "sensor.alarm.fire":
		$("#SettingsAddDeviceCommonRole").val("iQontrolFire").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "action.execute":
		$("#SettingsAddDeviceCommonRole").val("iQontrolProgram").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;

		case "scene.state":
		$("#SettingsAddDeviceCommonRole").val("iQontrolScene").selectmenu("refresh");
		settingsAddDeviceCommonRoleChanged();
		break;
	}
}

function settingsAddDeviceCommonRoleChanged(){
	var selectedRole = $("#SettingsAddDeviceCommonRole option:selected").val();
	$(".SettingsAddDeviceState").hide();
	$(".SettingsAddDeviceState." + selectedRole).show();
}
...
function settingsAddDevice(){
	...
	var obj = {
		...
		"common": {
			"desc": "created by iQontrol",
			"role": $("#SettingsAddDeviceCommonRole").val(),
			"name": $("#SettingsAddDeviceCommonName").val(),
			"icon": ""
		},
		"native": {
	...
	console.log("++++++Create object: " + JSON.stringify(obj));
	var settingsAddDeviceServerTransactionCounter = 0;
	(function(){ //Closure (everything declared inside keeps its value as ist is at the time the function is created)
		var selectedRole = $("#SettingsAddDeviceCommonRole option:selected").val();
		var _childStates = $(".SettingsAddDeviceState." + selectedRole + " input");
		var _objId = objId;
		var _obj = obj;
		settingsAddDeviceServerTransactionCounter++;
		setObject(_objId, _obj, function(error){
			settingsAddDeviceServerTransactionCounter--;
			if (error) alert("Error at creating object"); else {
				for(var i = 0; i < _childStates.length; i++){
					var childStateName = _childStates[i].name;
					childStateName = childStateName.substring(22, childStateName.length); // "SettingsAddDeviceState" length = 22
					if($("#" + childStateName) !== ""){
						var childId = objId + "." + childStateName;
						var childObj = {
							"_id": childId,
							"type": "state",
							"common": {
						...
						(function(){ //Closure (everything declared inside keeps its value as ist is at the time the function is created)
							var _childId = childId;
							var _childValue = $("#SettingsAddDeviceState" + childStateName).val();
							console.log("+++Create childState: " + JSON.stringify(childObj));
							settingsAddDeviceServerTransactionCounter++;
							setObject(childId, childObj, function(error){
								settingsAddDeviceServerTransactionCounter--;
								if (error) alert("Error at creating child"); else {
									console.log("++Set childState " + _childId + " --> " + _childValue);
									settingsAddDeviceServerTransactionCounter++
									setState(_childId, "", _childValue, true, function(error){
										settingsAddDeviceServerTransactionCounter--;
										if (error) alert("Error setting child value");
										if(settingsAddDeviceServerTransactionCounter < 1){
											getEverything();
											alert ("Object created. Database is reloading now. That may take a few seconds.");
										}
									});
		...
		switch(usedObjects[deviceId].common.role){
			...
			if(level){
				if(level.type == 'level'){
					...
					dialogContent += "<label for='DialogLevelSlider' ><image src='./images/slider.png' / style='width:16px; height:16px;'>&nbsp;" + _("Dimmer") + ":</label>";
